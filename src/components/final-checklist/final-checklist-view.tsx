...
export function FinalChecklistView() {
  ...
  const checklist = [
    { label: "Kalıcı veri katmanı", status: isApiMode ? "completed" : "partial", note: isApiMode ? "UI kalıcı veriyi API-first modda tüketiyor." : "Fallback modu halen aktif." },
    { label: "Auth ve session", status: auth.isAuthenticated ? "completed" : "partial", note: auth.isAuthenticated ? "DB-backed session aktif." : "Login + 2FA akışı hazır, yeni oturum bekleniyor." },
    { label: "Çekirdek API katmanı", status: isApiMode ? "completed" : "partial", note: "Assets, events, deception, reports ve settings endpointleri aktif." },
    { label: "Backend karar motorları", status: environment.events.length > 0 && hasZeroTrustDecisions ? "completed" : "partial", note: "Risk, Zero Trust, deception, compliance ve report kararları server-side üretiliyor." },
    { label: "Frontend API adaptörü", status: isApiMode ? "completed" : "partial", note: isApiMode ? "Store API üzerinden hydrate oluyor." : "Fallback modu korunuyor." },
    { label: "Trap endpoint", status: hasTrapEndpointAlert || environment.deceptions.length > 0 ? "completed" : "partial", note: hasTrapEndpointAlert ? "Güvenli trap endpoint alarmı audit log’a işlendi." : "Trap endpoint hazır, ilk probe bekleniyor." },
    { label: "Rapor yazdırma akışı", status: hasPrintedReport || hasReports ? "completed" : "partial", note: hasPrintedReport ? "Yazdırma akışı audit log’a işlendi." : "DB-backed print payload route hazır." },
    ...
    { label: "2FA var", status: auth.is2FAVerified ? "completed" : auth.isAuthenticated ? "partial" : "missing", note: auth.is2FAVerified ? "İkinci faktör doğrulaması tamamlandı." : "Doğrulama kodu ile ikinci faktör adımı bekleniyor." },
    ...
    { label: "IAM/MFA/RBAC/ABAC senaryo seti var", status: environment.simulations.length >= 4 ? "completed" : "partial", note: `${environment.simulations.length} senaryo yüklü.` },
    ...
    { label: "Guided run var", status: hasSimulationRun ? "completed" : "partial", note: hasSimulationRun ? "Uçtan uca operasyon çalıştırması audit log’a işlendi." : `${environment.demoScenario.steps.length} operasyon adımı hazır.` },
    ...
    { label: "Executive briefing var", status: lastSimulationResult || environment.demoScenario.active ? "completed" : "partial", note: "Executive briefing görünümü operasyon state’ini okuyabiliyor." },
  ...
  return (
    <Panel>
      <PageIntro
        ...
        description="Bu sayfa, ürünleşme yetkinliklerinin ve operasyon hazırlığının sistem üzerinde ne durumda olduğunu hızlıca gösterir."
      />
